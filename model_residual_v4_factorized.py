...
def create_model(input_shape=(None, None, 3), depths=[1, 2, 4, 8], width=32, sqrt_depth_multiplier=2, max_drs=[7, 7, 7, 5], activation='relu', interp_mode='bilinear'):
    ...
    for depth, max_dr in zip(depths, max_drs):
        ...
        for _ in range(depth):
            ...
            x = FilterResponseNormalization(activation)(x)
            x = FastDepthwiseContextConv2D(sqrt_depth_multiplier=sqrt_depth_multiplier, max_learned_dilation_rate=max_dr, interp_mode=interp_mode, kernel_initializer='he_uniform')(x)
            x = FilterResponseNormalization(activation)(x)
